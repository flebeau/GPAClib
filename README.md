...
In order to use GPAClib, you need to have:
  ...
  - Boost library (more specifically: System, Program Options, Iostreams and Spirit)
...
Operands are names of gates defined (before or after) in the same circuit or `t`. Gate names cannot be empty, cannot start with underscore and `t` is reserved.
